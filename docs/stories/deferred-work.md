...
- **Full VS Code Debug Adapter for cell debugging.** Story 2.5 (amended 2026-04-25) implements only the CDP-side mirror: VS Code-side notebook-cell breakpoints are forwarded to the page via `Debugger.setBreakpointByUrl` and pause execution in the browser, but pause inspection happens in the browser's DevTools. The VS Code editor does NOT show a solid "verified" gutter glyph, does NOT enter a `vscode.DebugSession`, does NOT highlight the paused line, and does NOT populate the Variables / Call Stack / Watch panels. Implementation validation also confirmed a Chromium/DevTools limitation: a breakpoint mirrored from the extension's separate CDP session can bind in V8 and trigger pauses without rendering a visible gutter marker in DevTools Sources. To deliver a real VS Code debug experience for notebook cells (solid gutter glyph, paused-line marker, Variables/Watch/Call Stack panels, step in/over/out, conditional breakpoints, exception breakpoints), the extension must register its own Debug Adapter Protocol (DAP) adapter that claims the `vscode-notebook-cell://` URI scheme and bridges DAP requests to the existing CDP Debugger/Runtime domains. vscode-js-debug cannot be reused because it has no extension point for foreign URI schemes (see decision history in this branch's chat record). This is sized as a new epic, not a single story, because the DAP surface (`setBreakpoints`, `threads`, `stackTrace`, `scopes`, `variables`, `continue`, `next`/`stepIn`/`stepOut`, `evaluate`, `stopped`, exception breakpoints, conditional breakpoints, lifecycle, plus reconciliation with the existing extension connection and DevTools coexistence) is substantially larger than any current Epic 2 story.

## Deferred from: code review of story-2-5 (2026-05-14)

- **Restore unconditional auto-resume in `BreakpointMirror.onPaused`.** Story 2.5's Locked Decision #2 and AC 5 require the extension to call `Debugger.resume` without inspecting `reason`, `hitBreakpoints`, or call frames. The shipped implementation in [src/debugger/breakpoint-mirror.ts](../../src/debugger/breakpoint-mirror.ts) reads `event.reason` and joins `event.hitBreakpoints` to emit a "Browser debugger pause observed on extension session" log line **before** dispatching `Debugger.resume`. This deviation was accepted as transitional diagnostic instrumentation: with no `vscode.DebugSession` and no DevTools gutter marker for mirrored breakpoints, the pause log is the only in-VS-Code evidence that the mirror binds and hits breakpoints. Bounded risk: the logger is a synchronous append to the extension output channel; a synchronous throw would prevent `resume()` from being dispatched and would hold the JS thread for any other CDP client. The Full VS Code Debug Adapter epic owns pause inspection as a first-class concern and MUST, on landing, restore the unconditional-resume contract by either (a) dispatching `resume()` first and logging afterward, or (b) deleting the pause log entirely if the new `vscode.DebugSession` provides equivalent visibility. The associated l10n key `"Browser debugger pause observed on extension session: {0}"` should be removed at the same time if no longer used.
