...
- Activation events: VS Code auto-adds `onCommand:*` for every contributed command, so the connect command is the implicit baseline — do NOT list `onCommand:jupyterBrowserKernel.connect` in `activationEvents` (it emits a redundancy warning). Epic 10 adds the explicit `onDebug:jupyter-browser-kernel` so the DAP adapter can activate when a debug session starts. Add additional activation events only when requirements change.
